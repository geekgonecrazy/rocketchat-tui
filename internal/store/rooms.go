...
//
// It replaces rather than merges: the list is a snapshot of who is in the room
// now, so someone who has left should stop being offered.
func (s *Store) SaveRoomMembers(roomID string, members []rocket.User) error {
	if roomID == "" {
		return nil
	}
	return s.withTx(func(tx *sql.Tx) error {
		if _, err := tx.Exec(`DELETE FROM room_members WHERE room_id = ?`, roomID); err != nil {
			return fmt.Errorf("store: clear members %s: %w", roomID, err)
		}
		stmt, err := tx.Prepare(`
			INSERT INTO room_members (room_id, username, name) VALUES (?,?,?)
			ON CONFLICT(room_id, username) DO UPDATE SET name = excluded.name`)
		if err != nil {
			return fmt.Errorf("store: prepare member insert: %w", err)
		}
		defer stmt.Close()
		...
		for _, member := range members {
			...
			if _, err := stmt.Exec(roomID, member.Username, member.Name); err != nil {
				return fmt.Errorf("store: save member %s: %w", member.Username, err)
			}
		}
		return nil
	})
...
// mentionCandidatesSQL unions the room's fetched members with everyone who has
// spoken in it, so the completer has candidates from cache alone — before any
// member fetch lands, and at all when offline.
//
// Recent speakers sort first: the person you are replying to is far more likely
// to be who you are mentioning than whoever happens to sort first alphabetically.
// Members who have never spoken carry no timestamp and fall through to the
// alphabetical tail.
const mentionCandidatesSQL = `
SELECT username, MAX(name), MAX(last_at) AS ranked_at FROM (
	SELECT room_id, username, name, 0 AS last_at FROM room_members WHERE room_id = ?
	UNION ALL
	SELECT room_id, username, author AS name, MAX(ts) AS last_at
	FROM messages WHERE room_id = ? AND username != '' GROUP BY username
)
WHERE username != ?
GROUP BY username
ORDER BY ranked_at DESC, username ASC`

// MentionCandidates returns everyone the composer may offer for a room,
// excluding the logged-in user.
func (s *Store) MentionCandidates(roomID string) ([]model.Member, error) {
	rows, err := s.db.Query(mentionCandidatesSQL, roomID, roomID, s.identityName())
	if err != nil {
		return nil, fmt.Errorf("store: mention candidates %s: %w", roomID, err)
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(&member.Username, &member.Name, &rankedAt); err != nil {
...
// SaveRoomMembers replaces a room's cached member list.
//
// It replaces rather than merges: the list is a snapshot of who is in the room
// now, so someone who has left should stop being offered.
func (s *Store) SaveRoomMembers(roomID string, members []rocket.User) error {
